...
async function main() {
  ...
  try {
    const response = await axios.get("https://ulinkcollege.engagehosted.cn/JS/PupilDetails.js", {
      headers: {
    ...
    fs.writeFileSync("PupilDetails.js", response.data);
    console.log("Saved to PupilDetails.js");
  } catch (err) {
    console.error(err);
  }
}
