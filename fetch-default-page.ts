...
async function main() {
  ...
  try {
    const response = await axios.get("https://ulinkcollege.engagehosted.cn/vle/default.aspx", {
      headers: {
    ...
    fs.writeFileSync("default-page.html", response.data);
    console.log("Saved to default-page.html");
  } catch (err) {
    console.error(err);
  }
}
